...
import hu.pappbalazs.test.gateway.config.EmbeddedKafka;
import hu.pappbalazs.test.gateway.config.KafkaSseConsumer;
import hu.pappbalazs.test.gateway.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class JhipsterTestGatewayKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/jhipster-test-gateway-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/jhipster-test-gateway-kafka/consume")

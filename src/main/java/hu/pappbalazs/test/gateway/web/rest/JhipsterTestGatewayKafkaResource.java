...
@RestController
@RequestMapping("/api/jhipster-test-gateway-kafka")
public class JhipsterTestGatewayKafkaResource {

    private final Logger log = LoggerFactory.getLogger(JhipsterTestGatewayKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public JhipsterTestGatewayKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
